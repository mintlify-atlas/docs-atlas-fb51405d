---
title: "HealtyHelp Environment Variables Reference"
sidebarTitle: "Environment Variables"
description: "Complete reference for all required and optional environment variables used by the HealtyHelp server and Vite client, including MongoDB, JWT, OAuth, Cloudinary, and email settings."
---
...
<ParamField path="BACKEND_URL" type="string" required>
  The public base URL of the server itself (no trailing slash). Used to build the Google OAuth callback URL inside `passport.js`.
...
These variables configure the `passport-google-oauth20` strategy in `server/config/passport.js`.
...
<ParamField path="GOOGLE_CALLBACK_URL" type="string">
  The full callback URL registered in Google Cloud Console. Must match exactly.
  Example: `https://api.your-domain.com/api/auth/google/callback`

  > Note: the passport strategy constructs the callback URL as `` `${process.env.BACKEND_URL}/api/auth/google/callback` ``, so `BACKEND_URL` must be set correctly instead of (or in addition to) this variable.
</ParamField>
