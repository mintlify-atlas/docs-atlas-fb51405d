...
Logs a recipe as consumed right now. The `tipo` is inferred automatically from the current Bogotá hour (see auto-detection table above). Pass `tipo` in the request body to override.
...
<ParamField body="tipo" type="string">
  Override the automatic meal slot: `desayuno`, `almuerzo`, `cena`, or `snack`. If omitted, the slot is inferred from the current hour.
</ParamField>
...
| `409` | Snack limit reached (max 3/day) or duplicate meal slot on the same day |
