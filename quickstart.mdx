...
<Steps>
  ...
  <Step title="Prerequisites">
    ...
    | **SMTP credentials** *(optional)* | Required for password-reset emails and admin invitation emails via Nodemailer. |
  </Step>
  ...
  <Step title="Configure Server Environment Variables">
    ...
    **Variable reference:**
    ...
    | `EMAIL_HOST` | Optional | SMTP host for Nodemailer (password resets, invitations). |
    | `EMAIL_PORT` | Optional | SMTP port (typically `587` for TLS). |
    | `EMAIL_USER` | Optional | SMTP username / sender address. |
    | `EMAIL_PASS` | Optional | SMTP password or app-specific password. |
  ...
  <Step title="Initialize the Super-Admin Account">
    Before you can access the admin panel, run the bootstrap script from the **project root** (or from `server/`). It reads `SUPER_ADMIN_EMAIL`, `SUPER_ADMIN_NAME`, and `SUPER_ADMIN_PASSWORD` from your `server/.env` and creates an admin user with `role: 'admin'` and `isSuperAdmin: true`:
