...
│  /api/terms  /api/notifications  /api/contacto  /api/utils       │
└────┬──────────────┬──────────────┬──────────────┬───────────────┘
     │              │              │              │
     ▼              ▼              ▼              ▼
 MongoDB        Groq AI       Cloudinary    Google OAuth
 Atlas          LLaMA 3.3     (images)      + Nodemailer
...
<CardGroup cols={2}>
  ...
  <Card title="Integrations" icon="plug">
    Groq AI (LLaMA 3.3 70B), Cloudinary (images), Google OAuth via Passport, Nodemailer (email).
  </Card>
</CardGroup>
...
| `/api/utils` | Utility endpoints (e.g., backend health ping) |
...
### Cloudinary
Recipe review images are uploaded to Cloudinary and stored with a status of `pendiente` until an admin approves or rejects them via the `ImagenesAprobacion` panel. Approved images return a public CDN URL; rejected ones are deleted from Cloudinary using the stored `publicId`.
...
### Nodemailer
Handles transactional email: password-reset tokens delivered to the user's address, and admin invitation links sent to prospective admins. SMTP credentials are read from `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USER`, and `EMAIL_PASS`.
