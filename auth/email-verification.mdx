...
description: "HealtyHelp requires a 6-digit email verification code after registration and supports token-based password reset via Resend."
---
...
<Steps>
  <Step title="Register an account">
    Submit a `POST /api/auth/register` request. The server generates a cryptographically random **6-digit numeric code**, hashes it with SHA-256, stores the hash and an expiry 15 minutes in the future, then sends the plaintext code to the user's email.
